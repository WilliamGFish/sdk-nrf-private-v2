...
#else
    /* Legacy Zephyr crypto API fallback - use HMAC as approximation */
    /* Note: This is not ideal as HMAC != CMAC, but may work for some use cases */
    if (!device_is_ready(g_crypto_dev)) {
        LOG_ERR("CMAC: Crypto device (%s) is not ready", g_crypto_dev->name);
        return -ENODEV;
    }

    /* For legacy API, we could potentially use HMAC-SHA256 and truncate */
    /* However, this is not cryptographically equivalent to CMAC */
    /* A proper CMAC implementation would be needed here */
    LOG_WRN("CMAC: Using legacy crypto API - CMAC not directly supported");
    LOG_WRN("CMAC: Consider enabling PSA crypto for proper CMAC support");
    
    /* Return error if PSA is not available and proper CMAC is required */
    return -ENOTSUP;
#endif
}
